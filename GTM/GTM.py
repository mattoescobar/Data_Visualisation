...
class GTM(object):
    def __init__(self, input_data=sp.rand(100, 4), rbf_number=25,
                 rbf_width=0.25, regularization=1, latent_space_size=100,
                 iterations=100):
        ...
        :param regularization: regularization parameter for maximum likelihood
        optimization
        :param latent_space_size: size of the grid, considering the total
    ...
    def gtm_pc_initialization(self):
        ...
        :return: beta: Initial scalar value of the inverse variance common to
        ...
        eigenvector = np.dot(pca.components_[:, 0:self.z.shape[0]],
        ...
        if self.z.shape[0] < self.input_data.shape[1]:
            beta = min(beta, 1 / pca.explained_variance_[self.z.shape[0] + 1])
        return w, beta
    ...
    def gtm_training(self):
        ...
        :return: beta: optimal scalar value of the inverse variance common to
        all components of the mixture
        :return: log_likelihood_evol: all log likelihood values for each cycle
        """
        [w, beta] = self.gtm_initialization()
        # Calculate Initial Distances
        self.gtm_distance = cdist(np.dot(self.fi, w), self.input_data,
                                  'sqeuclidean')
        # Training loop
        ...
        for i in xrange(0, self.iterations):
            ...
            lbda = self.regularization * np.ones((self.fi.shape[1],
                                                  self.fi.shape[1]))
            intermediate_matrix = np.dot(np.transpose(self.fi),
                                         np.diag(np.sum(self.gtm_responsibility,
                                                        1)))
            maximization_matrix = np.dot(intermediate_matrix, self.fi) + lbda \
                / beta
            inv_maximization_matrix = np.linalg.pinv(maximization_matrix)
            w = np.dot(inv_maximization_matrix, np.dot(np.transpose(self.fi),
    ...
    def gtm_mean(self, w, beta):
        """ Find mean probability density values for each sample in the latent
        space

        :param w: optimal weight matrix
        :param beta: optimal scalar value of the inverse variance common to
        all components of the mixture
        """
        self.gtm_distance = cdist(np.dot(self.fi, w), self.input_data,
                                  'sqeuclidean')
        self.gtm_responsibilities(beta)
        means = np.dot(np.transpose(self.gtm_responsibility), np.transpose(
            self.z))
        return means

    def gtm_mode(self, w):
        """ Find mode probability density values for each sample in the latent
        space
    ...
    def gtm_pdf(self):
        """  Plot gtm's probablity distribution in the form of a heat map for
        each sample in the latent space

        """
        lat_dim = np.sqrt(self.latent_space_size)
        plt.pcolor(np.reshape(self.z[0, :], (lat_dim, lat_dim)),
                   np.reshape(self.z[1, :], (lat_dim, lat_dim)),
                   np.reshape(np.sum(self.gtm_responsibility, 1),
                              (lat_dim, lat_dim)),
